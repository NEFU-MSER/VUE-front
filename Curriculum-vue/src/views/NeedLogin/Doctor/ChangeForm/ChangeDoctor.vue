...
const props = defineProps({
  ...
  roles: { type: Array<Role>, required: true },
  addMethod: { type: Boolean, default: false }
})
const change = ref(cloneDeep(props.doctor))
const disable = ref(true)
const roles = cloneDeep(props.roles)
const tips = ref('')
...
function isFull() {
  if (change.value.name.length <= 0) {
    disable.value = true
    tips.value = '姓名不可为空'
  } else if (change.value.roleId.length != 19) {
    disable.value = true
    tips.value = '职务分配出错'
...
<template>
  <h3>{{ addMethod ? '添加职务' : '修改' + props.doctor.name }}</h3>
  <el-form label-width="10%">
    ...
    <el-form-item label="职务">
      <el-select v-model="change.roleId" placeholder="请选择隶属科室">
    ...
    <el-form-item>
      ...
      <el-button
        ...
        @click="
          () => {
            change.email += select
            addMethod ? localDB.doctorData.add(change) : localDB.doctorData.update(change)
            emits('updated')
          }
        "
