...
async function addLib() {
  if (postForm.value._libType === '' || postForm.value._libId == 0) {
    await ElMessageBox.alert('教室编号不能为0 且 教室类型不能为空。', '警告')
  } else {
    await axios({
      method: 'POST',
      ...
      .then(async (res) => {
        ...
        await ElMessageBox.alert(response.reason, response.result ? '提示' : '警告')
        if (response.result) {
          location.reload()
        }
      })
...
<template>
  <el-row>
    ...
    <el-col :span="8">
      <el-select v-model="lib" placeholder="Select" size="large" class="input" @change="setAdd">
  ...
  <el-card style="max-width: 800px" v-if="lib._libId != -1 && !add">
    <template #header>
      <div class="card-header">
        <span>实验室: {{ lib._libId }} {{ lib._libType }}</span>
      </div>
    </template>
    <el-table :data="lib._libOccupations" stripe style="width: 100%">
      <el-table-column prop="_course._courseName" label="课程名" />
      <el-table-column prop="_course._teacherName" label="授课人" />
      <el-table-column prop="_timeDescribe" label="授课时间" />
    </el-table>
  </el-card>
